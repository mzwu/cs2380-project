...
if __name__ == "__main__":
    ...
    # Format data for equal shares function
    voters = [f"v{i}" for i in range(len(approvals))]
    projects = [*project_costs.keys()]
    cost = project_costs
    approvers = {p: [] for p in projects}
    ...
    print("Chosen projects:", winners)

    # get total utility
    util = sum(len(approvers[p]) for p in winners)
    print("Total utility:", util)

    # get total cost
    total_cost = sum(cost[p] for p in winners)
    print("Total cost used:", total_cost)

    data = {
        "chosen_projects": list(winners),
        "utility": util,
