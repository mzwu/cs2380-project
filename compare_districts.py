...
def create_comparison_charts():
    ...
    wesola_utility_per_voter = [r['utility'] / wesola['num_voters'] for r in wesola['results']]
    marysin_utility_per_voter = [r['utility'] / marysin['num_voters'] for r in marysin['results']]
    ...
    ax1.set_ylim(0, max(max(wesola_utility_per_voter), max(marysin_utility_per_voter)) * 1.2)
    ...
    wesola_projects_pct = [r['num_projects'] / wesola['num_projects'] * 100 for r in wesola['results']]
    marysin_projects_pct = [r['num_projects'] / marysin['num_projects'] * 100 for r in marysin['results']]
    ...
    ax3.set_title('Percentage of Projects Funded by Method', fontsize=16, fontweight='bold', pad=20)
    ax3.set_xticks(x)
    ...
    overview_data = [
        ...
        ['Year', '2023', '2026'],
        ['Budget', f"{wesola['total_budget']:,}", f"{marysin['total_budget']:,}"],
        ['Voters', f"{wesola['num_voters']:,}", f"{marysin['num_voters']:,}"],
        ['Projects', str(wesola['num_projects']), str(marysin['num_projects'])],
    ...
    for district_name, data in [('Wesoła', wesola), ('Marysin', marysin)]:
        ...
        best_efficiency = min(results, key=lambda x: x['unused_percentage'])['method']
        ...
        best_methods.append([district_name, best_utility.replace(' PB', '').replace('Method of ', ''), 
                           best_efficiency.replace(' PB', '').replace('Method of ', ''),
                           best_projects.replace(' PB', '').replace('Method of ', '')])
    
    best_data = [['District', 'Best Utility', 'Best Efficiency', 'Most Projects']] + best_methods
    ax.axis('off')
    ...
    table2.scale(1.2, 2)
    ax.set_title('Best Performing Method per Metric', fontsize=14, fontweight='bold', pad=20)
    
    # Panel 3: Method Ranking by District
    ax = axes[1, 0]
    
    # Calculate average normalized score per method per district
    wesola_avg = [(norm_utility[i] + norm_efficiency[i] + norm_projects[i]) / 3 for i in range(3)]
    marysin_avg = [(norm_utility[i+3] + norm_efficiency[i+3] + norm_projects[i+3]) / 3 for i in range(3)]
    
    x = np.arange(len(method_short))
    bars1 = ax.bar(x - width/2, wesola_avg, width, label='Wesoła', color='#e74c3c', edgecolor='black')
    bars2 = ax.bar(x + width/2, marysin_avg, width, label='Marysin', color='#3498db', edgecolor='black')
    
    ax.set_ylabel('Average Score', fontsize=12, fontweight='bold')
    ax.set_title('Overall Method Performance\n(Average of Normalized Metrics)', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    if avg_wesola_eff < avg_marysin_eff:
        insights.append(f"• Wesoła has better avg efficiency ({avg_wesola_eff:.1f}% vs {avg_marysin_eff:.1f}%)")
    else:
        insights.append(f"• Marysin has better avg efficiency ({avg_marysin_eff:.1f}% vs {avg_wesola_eff:.1f}%)")
    
    # EJR compliance
    insights.append("• All methods satisfy EJR in both districts")
    
    insights_text = '\n\n'.join(insights)
    ax.text(0.1, 0.8, "Key Insights", fontsize=14, fontweight='bold', transform=ax.transAxes)
